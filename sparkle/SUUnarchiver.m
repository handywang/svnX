...
//SMI support requires you to have your file distributed with macbinary encoding in order to preserve the resource fork
- (BOOL)_extractSMI:(NSString *)archivePath
{	
	//First, we convert from smi to dmg so we can internet-enable the image
	NSString *newArchivePath = [archivePath stringByAppendingString:@".dmg"];
	NSTask *hdiTask = [NSTask launchedTaskWithLaunchPath:@"/usr/bin/env" arguments:[NSArray arrayWithObjects:@"hdiutil", @"convert", @"-quiet", @"-format", @"UDZO", @"-o", newArchivePath, archivePath, nil]];
	[hdiTask waitUntilExit];
	if ([hdiTask terminationStatus] != 0) { return NO; }
	
	return [self _extractDMG:newArchivePath];
}
