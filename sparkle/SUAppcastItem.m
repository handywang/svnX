...
- initWithDictionary:(NSDictionary *)dict
{
	...
	//unescape things like &amp; so developers can use & in their download url
	CFStringRef url = CFXMLCreateStringByUnescapingEntities(kCFAllocatorDefault, (CFStringRef)[enclosure objectForKey:@"url"], NULL);
	[self setFileURL:[NSURL URLWithString:[(NSString*)url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
	if(url)
		CFRelease(url);
